...
ru.akman.znotes.tests.TestSuite = function() {
  ...
  var log = Utils.getLogger( "content.tests.testsuite" );
  ...
  tests.push( {
    name: "XBL Test",
    description: "XBL Bindings test",
    code: function () {
      var aWindow = window.open(
        "chrome://znotes/content/testsuite/xbltest.xul",
        "znotes:test_xbltest",
        "chrome,toolbar,status,resizable,centerscreen"
      );
      aWindow.addEventListener( "close", function( event ) {
        openWindows.splice( openWindows.indexOf( aWindow ), 1 );
      }, true );
  ...
  tests.push( {
    name: "Notes tree",
    description: "Custom view for notes tree",
    code: function () {
      var aWindow = window.open(
        "chrome://znotes/content/testsuite/notestree.xul",
        "znotes:test_notestree",
        "chrome,toolbar,status,resizable,centerscreen"
      );
      aWindow.addEventListener( "close", function( event ) {
        openWindows.splice( openWindows.indexOf( aWindow ), 1 );
  ...
  tests.push( {
    ...
    code: function () {
      ...
      // XXX ??? anURI cause message in error console, what principal must be use?
  ...
  tests.push( {
    name: "Ordered insert",
    description: "Inserting into a sorted array",
    code: function () {
      ...
      function search( arr, value, start, end ) {
        var index, flag, order = compare( arr[end], arr[start] );
        if ( order === 0 ) {
          flag = compare( value, arr[start] );
          return ( flag < 0 ? start : end + 1 );
        }
        while ( start < end ) {
          index = Math.round( ( start + end ) / 2 );
          flag = order * compare( value, arr[index] );
          if ( flag < 0 ) {
            end = index - 1;
          } else {
            start = index + 1;
      ...
      function insert( arr, value, start, end ) {
        var a = arr.slice( 0 );
        log.trace( "source: " + a + " , value: " + value );
        a.splice( search( a, value, start, end ), 0, value );
        log.trace( "result: " + a );
      };
      
      var arr1 = [  2,  4,  6,  7, 10, 14, 17, 19, 21 ];
      //            0   1   2   3   4   5   6   7   8
      insert( arr1,  1, 0, arr1.length - 1 );
      insert( arr1,  2, 0, arr1.length - 1 );
      insert( arr1,  3, 0, arr1.length - 1 );
      insert( arr1,  4, 0, arr1.length - 1 );
      insert( arr1, 19, 0, arr1.length - 1 );
      insert( arr1, 20, 0, arr1.length - 1 );
      insert( arr1, 21, 0, arr1.length - 1 );
      insert( arr1, 22, 0, arr1.length - 1 );

      var arr2 = [ 21, 19, 17, 14, 10,  7,  6,  4,  2 ];
      //            0   1   2   3   4   5   6   7   8
      insert( arr2,  1, 0, arr2.length - 1 );
      insert( arr2,  2, 0, arr2.length - 1 );
      insert( arr2,  3, 0, arr2.length - 1 );
      insert( arr2,  4, 0, arr2.length - 1 );
      insert( arr2, 19, 0, arr2.length - 1 );
      insert( arr2, 20, 0, arr2.length - 1 );
      insert( arr2, 21, 0, arr2.length - 1 );
      insert( arr2, 22, 0, arr2.length - 1 );

      var arr3 = [ 23, 23, 23, 23, 23, 23, 23, 23, 23 ];
      //            0   1   2   3   4   5   6   7   8
      insert( arr3, 20, 0, arr3.length - 1 );
      insert( arr3, 23, 0, arr3.length - 1 );
      insert( arr3, 25, 0, arr3.length - 1 );
  ...
  tests.push( {
    ...
    code: function () {
      var intervals = DateUtils.getDateIntervals( 1 );
      function show( d ) {
        intervals.forEach( function( interval ) {
          var r = interval.range( d );
          log.trace( interval + ": " +
            r.start.toLocaleString() + " - " + r.end.toLocaleString() );
        } );
        log.trace( "" );
      };
      intervals.firstDayOfWeek = 1;
      show( new Date( 2015, 1, 15, 12, 0, 0 ) );
      show( new Date( 2015, 4, 15, 12, 0, 0 ) );
      show( new Date( 2015, 7, 15, 12, 0, 0 ) );
      show( new Date( 2015, 10, 15, 12, 0, 0 ) );
      intervals.firstDayOfWeek = 0;
      show( new Date( 2015, 1, 15, 12, 0, 0 ) );
      show( new Date( 2015, 4, 15, 12, 0, 0 ) );
      show( new Date( 2015, 7, 15, 12, 0, 0 ) );
      show( new Date( 2015, 10, 15, 12, 0, 0 ) );
      intervals.firstDayOfWeek = 6;
      show( new Date( 2015, 1, 15, 12, 0, 0 ) );
      show( new Date( 2015, 4, 15, 12, 0, 0 ) );
      show( new Date( 2015, 7, 15, 12, 0, 0 ) );
      show( new Date( 2015, 10, 15, 12, 0, 0 ) );
...
window.addEventListener( "load", ru.akman.znotes.tests.TestSuite.onLoad, false );
window.addEventListener( "unload", ru.akman.znotes.tests.TestSuite.onUnload, false );
window.addEventListener( "close", ru.akman.znotes.tests.TestSuite.onClose, false );
